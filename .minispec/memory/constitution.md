...
Use Spec Kit or MiniSpec for non-trivial features.
Spec Kit flow: `/speckit-specify`, then plan, then tasks, then implement, then converge.
MiniSpec flow: `/minispec.design`, then tasks, then implement.
...
MiniSpec lives beside Spec Kit in this repo.
Spec Kit artifacts stay in `specs/<id>-<name>/` as `spec.md`, `plan.md`, and `tasks.md`.
MiniSpec artifacts in a feature directory are `design.md` and `tasks.md`.
Do not mix both workflows in the same feature directory.
Use `/minispec.import` to convert a Spec Kit spec into MiniSpec artifacts.
...
review-decisions (engineer reviews `decisions/`; AI may write patterns/modules)

### Autonomy Level

familiar-areas (proceed in code already reviewed this session; pause for new areas)
...
This constitution guides Spec Kit plans, MiniSpec designs, and tasks.
...
MiniSpec preferences can change per feature when the team agrees.
