...
function setupIntervalOrbit() {
  ...
  const tabs = document.createElement('div');
  tabs.id = 'io-tabs';
  insertBefore(layout, tabs, main);

  const setup = document.createElement('div');
  setup.id = 'io-setup';
  setup.className = 'mobile-setup-first';
  insertBefore(main, setup, main.firstChild);

  const compact = document.createElement('div');
  compact.id = 'io-compact-progress';
  compact.className = 'compact-progress auto-hide';
  const ph = main.querySelector('.progress-header');
  if (ph) ph.after(compact);

  const opts = main.querySelector('details.adv-options');
  if (opts) opts.open = false;

  const board = main.querySelector('.io-card');
  const improv = document.getElementById('io-improv-panel');
  const stats = main.querySelector('.io-stats-card');
  const dock = main.querySelector('.action-dock');
  const badge = document.getElementById('io-tuning-badge');

  // Progress is its own panel (moved after tabs area)
  if (stats) {
    const progressPanel = wrapAsSubview([stats], { id: 'progress', forTabs: 'io-tabs', active: false });
    // Keep at end of main
  }

  // Drill-only chrome (progress + options above board; dock after board)
  const drillOnly = document.createElement('div');
  drillOnly.className = 'subview-panel active io-drill-only';
  drillOnly.dataset.subview = 'drill';
  drillOnly.dataset.subviewFor = 'io-tabs';
  [ph, compact, opts].filter(Boolean).forEach(n => drillOnly.appendChild(n));
  if (board) main.insertBefore(drillOnly, board);

  const drillDock = document.createElement('div');
  drillDock.className = 'subview-panel active io-drill-dock';
  drillDock.dataset.subview = 'drill';
  drillDock.dataset.subviewFor = 'io-tabs';
  if (dock) {
    drillDock.appendChild(dock);
    if (board) board.after(drillDock);
    else main.appendChild(drillDock);
  }

  // Improv controls panel (board stays shared below)
  const improvOnly = document.createElement('div');
  improvOnly.className = 'subview-panel io-improv-only';
  improvOnly.dataset.subview = 'improv';
  improvOnly.dataset.subviewFor = 'io-tabs';
  improvOnly.hidden = true;
  if (improv) {
    improv.hidden = false;
    improvOnly.appendChild(improv);
    if (board) main.insertBefore(improvOnly, board);
    else main.appendChild(improvOnly);
  }

  // Shared setup + badge + board always visible for drill/improv
  if (setup && board) main.insertBefore(setup, drillOnly);
  if (badge && board) main.insertBefore(badge, board);

  // Hide shared board on progress tab via CSS class
  initSubviewTabs(tabs, [
    ...
    defaultId: 'drill',
    onChange: (id) => {
      sec.classList.toggle('io-mode-progress', id === 'progress');
      sec.classList.toggle('io-mode-improv', id === 'improv');
      if (board) board.hidden = id === 'progress';
      if (setup) setup.hidden = id === 'progress';
      if (badge) badge.hidden = id === 'progress';
      if (id === 'improv') {
        document.querySelector('#sl-io-drill .sl-item[data-val="improv"]')?.click();
      } else if (id === 'drill') {
        const active = document.querySelector('#sl-io-drill .sl-item.active');
        if (active?.dataset.val === 'improv') {
          (document.querySelector('#sl-io-drill .sl-item[data-val="find"]') ||
            document.querySelector('#sl-io-drill .sl-item:not([data-val="improv"])'))?.click();
        }
      }
      refreshIoSetup();
    },
  ...
  if (!document.getElementById('io-subview-style')) {
    const style = document.createElement('style');
    style.id = 'io-subview-style';
    style.textContent = `
      #sec-intervalorbit.io-mode-progress .io-card,
      #sec-intervalorbit.io-mode-progress #io-setup,
      #sec-intervalorbit.io-mode-progress #io-tuning-badge { display:none !important; }
    `;
    document.head.appendChild(style);
  }

  const tonal = document.getElementById('io-tonal-center');
  if (tonal && !document.getElementById('io-key-pick')) {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.id = 'io-key-pick';
    btn.className = 'setup-chip';
    btn.innerHTML = `<span class="setup-chip-value">${tonal.value || 'C'}</span><span class="setup-chip-hint">Key</span>`;
    tonal.style.display = 'none';
    tonal.parentNode.insertBefore(btn, tonal);
    btn.onclick = async () => {
      const next = await openRootPicker({ value: tonal.value, syncContext: false, source: 'io-improv' });
      if (next) {
        tonal.value = next;
        tonal.dispatchEvent(new Event('change', { bubbles: true }));
        btn.querySelector('.setup-chip-value').textContent = next;
      }
    };
  }

  const customProg = document.getElementById('io-prog-custom');
  if (customProg && !document.getElementById('io-edit-prog')) {
    const label = customProg.closest('label') || customProg.parentNode;
    label.hidden = true;
    const edit = document.createElement('button');
    edit.type = 'button';
    edit.id = 'io-edit-prog';
    edit.className = 'btn sm';
    edit.textContent = 'Edit Progression';
    edit.onclick = () => { label.hidden = !label.hidden; };
    label.parentNode.insertBefore(edit, label);
  }

  refreshIoSetup();
  wireDrillFocus('sec-intervalorbit', 'io');
}

function refreshIoSetup() {
  const el = document.getElementById('io-setup');
  if (!el) return;
  const tuning = getSetting('io.tuning', 'Standard');
  const drill = getSetting('io.drill', document.querySelector('#sl-io-drill .sl-item.active')?.dataset.val || 'find');
  const orbit = getSetting('io.orbit', document.querySelector('#sl-io-orbit .sl-item.active')?.dataset.val || 'medium');
  renderSetupSummary(el, [
    {
      ...
      onClick: async () => {
        const next = await openTuningPicker({
          ...
          includeCustom: true,
          onCustom: () => {
            const row = document.getElementById('io-custom-tuning-row');
            const opts = document.querySelector('#sec-intervalorbit details.adv-options');
            if (opts) opts.open = true;
            if (row) row.hidden = false;
          },
        });
        if (next) {
          const item = document.querySelector(`#sl-io-tuning .sl-item[data-val="${CSS.escape(next)}"]`);
          if (item) item.click();
          saveSetting('io.tuning', next);
          refreshIoSetup();
        }
      },
    },
    {
      key: 'drill', value: String(drill).replace(/-/g, ' '), hint: 'Drill',
      onClick: () => openIoListPicker('Drill', 'sl-io-drill', 'io.drill'),
    },
    {
      key: 'orbit', value: String(orbit), hint: 'Orbit',
      onClick: () => openIoListPicker('Orbit size', 'sl-io-orbit', 'io.orbit'),
    },
    {
      key: 'more', value: 'Options', hint: 'More',
      onClick: () => {
        const opts = document.querySelector('#sec-intervalorbit details.adv-options');
...
async function openIoListPicker(title, listId, settingKey) {
  const items = [...document.querySelectorAll(`#${listId} .sl-item`)].map(el => ({
    id: el.dataset.val,
    label: el.textContent.trim(),
  }));
  const current = document.querySelector(`#${listId} .sl-item.active`)?.dataset.val;
  const next = await openSelectionSheet({ title, items, value: current, search: items.length > 8 });
  if (next) {
    const item = document.querySelector(`#${listId} .sl-item[data-val="${CSS.escape(next)}"]`);
    if (item) item.click();
